...
def gen_secrets(channels: list[int]) -> dict:
    """Generate a secrets dictionary with channel keys and a master secret_key."""
    ...
    secrets_dict["secret_key"] = secrets.token_bytes(16).hex()
    return secrets_dict

def write_secrets_header(secrets_data: dict, header_file: Path):
    """Write secrets data directly to a C header style file."""
    try:
        channel_keys = secrets_data["channel_keys"]
        secret_key = secrets_data["secret_key"]
        
        all_keys_c = [f"{{ {', '.join(f'0x{b:02X}' for b in bytes.fromhex(key))} }}" for key in channel_keys.values()]
        secret_key_hex = ', '.join(f"0x{b:02X}" for b in bytes.fromhex(secret_key))
        ...
        with open(header_file, "w") as f:
            f.write("#ifndef SECRETS_H\n")
            f.write("#define SECRETS_H\n\n")
            f.write("static const uint8_t channel_keys[110][16] = {\n")
        ...
        logger.error(f"Failed to write secrets header: {e}")

def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("global_secrets_file", type=Path, help="Path to the global secrets header file to be created")
    parser.add_argument("channels", nargs="+", type=int, help="Supported channels. Channel 0 is always valid and not provided in this list")
    return parser.parse_args()

def main():
    args = parse_args()
    secrets_data = gen_secrets(args.channels)
    write_secrets_header(secrets_data, args.global_secrets_file)
