...
def gen_secrets(channels: list[int]) -> bytes:
    ...
    for channel_id in range(0, 10007):  # Assuming you need 10,007 keys
        ...
        secrets_dict["secret key"] = secret_key
        
    
    
    
    all_keys_c = [f"{{ {', '.join(f'0x{b:02X}' for b in bytes.fromhex(secrets_dict['channel_keys'][k]))} }}" for k in secrets_dict['channel_keys']]
