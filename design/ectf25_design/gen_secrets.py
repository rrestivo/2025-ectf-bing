...
def gen_secrets(channels: list[int]) -> bytes:
    ...
    Channel 0 is the emergency broadcast, which will always be valid and will NOT be included in this list.

    :returns: Contents of the secrets file.
    """
    # Dictionary to store channel-specific keys and a separate entry for the secret_key
    secrets_dict = {
        "channel_keys": {},
        "secret_key": ""
    }
    ...
    for channel in channels:
        ...
        secrets_dict["channel_keys"][str(channel)] = channel_key.hex()  # Store as hex string
...
def generate_secrets_header(secrets_file: Path, header_file: Path):
    ...
    try:
        ...
        # Extract channel keys and the secret key from the dictionary
        ...
        with open(header_file, "w") as f:
            ...
            f.write("    uint32_t channel_id;\n")
            f.write("    uint8_t key[16];\n")
            f.write("} ChannelKey;\n\n")

            # Write the array of channel keys
            f.write("static const ChannelKey channel_keys[] = {\n")
            for channel_id, key in channel_keys.items():
                key_hex = ", ".join(f"0x{b:02X}" for b in bytes.fromhex(key))
                f.write(f"    {{{channel_id}, {{{key_hex}}}}},\n")
            f.write("};\n\n")
