...
class Encoder:
    ...
    def encode(self, channel: int, frame: bytes, timestamp: int) -> bytes:
        """Encode a frame"""
        #logg.info(f"Encoding frame for Channel = {channel}, Timestamp = {timestamp}")

        if len(frame) > 64:
            raise ValueError("Frame size must not exceed 64 bytes.")

        #logg.debug(f"Frame length before padding: {len(frame)} bytes")

        # Calculate effective channel ID using modulo 10007
        effective_channel = channel % 10007
        #logg.debug(f"Effective channel ID after modulo: {effective_channel}")
        ...
        #logg.info(f"Using encryption key for effective channel {effective_channel}: {channel_key.hex()}")

        #------- Step 4: First encryption layer on (padded frame)
        encrypted_frame = self._encrypt(frame, channel_key)
        #logg.success(f"First encryption complete for channel {channel}. Encrypted Frame Size: {len(encrypted_frame)} bytes")

        # ---------- Step 5: Pack header (channel_id + timestamp + padd_done)
        header = struct.pack("<IQI", channel, timestamp, frame_size)
        #logg.debug(f"Packed Header Structure (Hex): {header.hex()} (Channel = {channel}, Timestamp = {timestamp}, frame size = {frame_size})")

        #---------- Step 6: Create full packet
        full_packet = header + encrypted_frame
        #logg.debug(f"Full Packet Length Before Final Padding: {len(full_packet)} bytes")
        ...
        #logg.info(f"Final Packet Size After Padding: {len(full_packet)} bytes (Multiple of 16)")
        #logg.debug(f"Final Packet Before Second Encryption (Hex): {full_packet.hex()}")

        # Step 8: Second encryption on (header + encrypted_frame)
        encrypted_packet = self._encrypt(full_packet, self.secret_key)
        #logg.success(f"Final encryption complete using secret_key. Final Encrypted Packet Size: {len(encrypted_packet)} bytes")
        #logg.debug(f"Final Encrypted Packet Structure (Hex): {encrypted_packet.hex()}")
