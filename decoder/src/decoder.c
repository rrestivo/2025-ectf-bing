...
int update_subscription(pkt_len_t pkt_len, uint8_t *update) {
    ...
    // Log the received packet length and check if it is a multiple of 16
    sprintf(debug_buf, "Received Packet Length: %d (Should be multiple of 16)", pkt_len);

    // Decrypt the incoming packet with error handling
    ...
    // Trim the decrypted packet to the expected size
    ...
    // Print the decoded subscription
