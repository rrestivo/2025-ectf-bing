...
int decode(pkt_len_t pkt_len, frame_packet_t *new_frame) {
    ...
    sprintf(output_buf, "Received Packet Length: %d bytes", pkt_len);
    print_debug(output_buf);

    // Print actual frame size
    sprintf(output_buf, "Expected frame_packet_t size: %d bytes", (int)sizeof(frame_packet_t));
    print_debug(output_buf);
    ...
    uint8_t decrypted_frame[pkt_len];  // Buffer to hold decrypted data
    uint8_t decrypted_message[FRAME_SIZE];  // Buffer to hold decrypted message

    print_debug("----------------------- we are after colour change -------------------------");

    // Debugging size before decryption
    sprintf(output_buf, "Size before decrypt_sym: %d bytes", pkt_len);
    print_debug(output_buf);

    print_debug("------ Entering decrypt_sym function ------");
    
    if (decrypt_sym((uint8_t *)new_frame, pkt_len, key, decrypted_frame) != 0) {
    ...
    frame_packet_t *decrypted_packet = (frame_packet_t *)frame_start;

    uint16_t frame_size = pkt_len - (sizeof(decrypted_packet->channel) + sizeof(decrypted_packet->timestamp));

    sprintf(output_buf, "Calculated Frame Size: %d bytes", frame_size);
    print_debug(output_buf);
